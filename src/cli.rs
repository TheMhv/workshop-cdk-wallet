...
use crate::lnd::LND;
use crate::{
    create_token, create_wallet, melt_quote, mint_info, mint_quote, mint_tokens, pay_mint_quote,
};
...
enum Commands {
    ...
    PayInvoice { quote_id: String, invoice: String },
    ...
    /// Melt token to pay an invoice
    MeltToken { quote_id: String, token: String },
...
pub async fn cli(mint_url: MintUrl, mut lnd: LND) -> anyhow::Result<()> {
    ...
    match args.command {
        ...
        Commands::PayInvoice { quote_id, invoice } => {
            let wallet = wallet(&mint_url).await?;
            let payment = lnd.pay_invoice(invoice).await?;

            pay_mint_quote(&wallet, &quote_id, payment).await?;
        }
        ...
        Commands::MeltToken { quote_id, token } => {
            let wallet = wallet(&mint_url).await?;
            let keyset = wallet.fetch_active_keyset().await?;

            let token = Token::from_str(&token)?;
            let proofs = token.proofs(&[keyset])?;
